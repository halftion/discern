...
// MAIN
  ...
  render(){
    return (
      <div style={styles.bg}>
        <Menu style={styles.menu} mode="horizontal" className="topmenu">
          <Menu.Item key="/homepage">
            <Link to="/homepage" style={{color:'#ccc'}}>
              首页
            </Link>
          </Menu.Item>
          <Menu.Item key="app">
            <Link to="/controlboard" style={{color:'#ccc'}}>
              监控台
            </Link>
          </Menu.Item>
          <Menu.Item key="/search">
            <Link to="/search" style={{color:'#ccc'}}>
              搜索
            </Link>
          </Menu.Item>
        </Menu>
        <canvas id="spring-text" style={styles.can}/>
        <Search placeholder="物 联 网 资 产 搜 索 引 擎" onSearch={this.onSearch} className="search" />
      </div>
    )
  }
}
...
const styles = {
  bg:{
    ...
    width: '100%',
    height: 'calc(100vh)',
    backgroundImage:`url("https://i.loli.net/2021/01/24/Slk3EaudCJiqPUM.png")`,
